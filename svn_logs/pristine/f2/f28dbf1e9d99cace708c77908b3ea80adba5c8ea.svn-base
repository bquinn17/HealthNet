...
class PatientRegistrationForm(forms.ModelForm):
    ...
    def clean_insurance_id(self):
        data = self.cleaned_data['insurance_id']
        if any((character == "<" or character == ">" or character == ";") for character in data):
            raise forms.ValidationError("You used illegal characters.")
        return data

    def clean_insurance_provider(self):
        data = self.cleaned_data['insurance_provider']
        if any((character == "<" or character == ">" or character == ";") for character in data):
            raise forms.ValidationError("You used illegal characters.")
        return data
...
class PatientUpdateForm(forms.ModelForm):
    ...
    class Meta:
        model = Patient
        fields = ('first_name', 'last_name', 'phone_number', "main_hospital", 'emergency_contact_name',
...
class AppointmentForm(forms.ModelForm):
    ...
    end_date = forms.DateTimeField(initial=str(timezone.now() + timezone.timedelta(hours=2))[:19])
...
class PatientAppointment(AppointmentForm, forms.ModelForm):
    ...
    end_date = forms.DateTimeField(widget=DateTimeWidget(),
                                   initial=str(timezone.now() + timezone.timedelta(hours=2))[:16])
...
class DoctorAppointment(AppointmentForm, forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        self.doctor = Doctor.objects.get(id=int(kwargs.pop('doctor')))
        super(DoctorAppointment, self).__init__(*args, **kwargs)
        hospital_list = []
        if self.doctor is not None:
            for hospital in self.doctor.hospitals.all():
...
class LogSearchForm(forms.Form):
    ...
    user_email = forms.EmailField(max_length=50, required=False,
                                  widget=forms.EmailInput(
                                      attrs={'class': 'form-control', 'placeholder': "user@email.com"}))
...
class AdminCreationForm(forms.Form):
    ...
    email = forms.EmailField(max_length=100, required=True,
                             widget=forms.EmailInput(attrs={'class': 'form-control', 'placeholder': "user@email.com"}))
...
class DoctorCreationForm(forms.ModelForm):
    ...
    email = forms.EmailField(max_length=50, required=True,
                             widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': "user@email.com"}))
...
class HospitalCreationForm(forms.ModelForm):
    ...
    hospital_name = forms.CharField(max_length=50, required=True,
                                    widget=forms.TextInput(
                                        attrs={'class': 'form-control', 'placeholder': "Healy McHospital"}))
...
class PrescriptionForm(forms.ModelForm):
    ...
    prescription_name = forms.CharField(max_length=150, required=True,
                                        widget=forms.TextInput(
                                            attrs={'class': 'form-control', 'placeholder': "Placebo 200mg"}))

    def clean_prescription_name(self):  # doesn't allow numbers in last name
        data = self.cleaned_data['prescription_name']
        if any((character == "<" or character == ">" or character == ";") for character in data):
            raise forms.ValidationError("You used illegal characters.")
        return data
...
class DoctorTransferPatientForm(forms.Form):
    """
    class_object: DoctorTransferPatientForm(forms.Form)
    Sub_Class:Meta
    Methods: clean_reason
    """
    hospital = forms.ModelChoiceField(queryset=Hospital.objects.all(), required=True, widget=forms.Select(
...
class DocEditForm(forms.ModelForm):
    """
    class_object: DoctorEditForm(forms.Form)
    Sub_Class:Meta
    Methods: clean_doctor_email
    """
    class Meta:
        model = Doctor
    ...
    doctor_email = forms.EmailField(max_length=100, required=False, disabled=False, widget=forms.TextInput(
        attrs={'class': 'form-control', 'placeholder': "Email of doctor whose hospitals to change"}))
...
class NurseEditForm(forms.ModelForm):
    """
    class_object: NurseEditForm(forms.Form)
    Sub_Class:Meta
    Methods: clean_nurse_email
    """

    class Meta:
        model = Nurse
        fields = ('nurse_email', 'working_hospital')

    working_hospital = forms.ModelChoiceField(queryset=Hospital.objects.all(), required=True,
                                              widget=forms.Select(attrs={'class': 'form-control'}))

    nurse_email = forms.EmailField(max_length=100, required=False, disabled=False, widget=forms.TextInput(
        attrs={'class': 'form-control', 'placeholder': "Email of nurse whose working hospital to change"}))

    def __init__(self, *args, **kwargs):
        status = kwargs.pop('status')
        self.nurse = kwargs.pop('nurse')
        super(NurseEditForm, self).__init__(*args, **kwargs)
        if status == "search":
            self.fields['working_hospital'].widget = self.fields['working_hospital'].hidden_widget()
            self.fields['working_hospital'].required = False
        if status == "edit":
            self.fields['nurse_email'].initial = self.nurse.username
            self.fields['nurse_email'].widget = self.fields['nurse_email'].hidden_widget()

    def clean_nurse_email(self):  # checks if they changed doctor's email if they somehow edit doctor_email
        #  even though it's hidden during editing of hospitals.
        nurse_email = self.cleaned_data['nurse_email']
        if self.nurse:
            if self.nurse.username != nurse_email:
                raise forms.ValidationError("You changed the nurse's email while editing hospitals.")
        return nurse_email
